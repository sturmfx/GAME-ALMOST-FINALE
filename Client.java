...
public class Client extends JFrame implements KeyListener
{
    ...
    static ArrayList<Bullet> temp_bullets2 = new ArrayList<>();
    ...
    final int bot_rate_of_fire = 1002;
    final int bot_distance_to_shoot;
            ...
            public void run()
            {
                if(go)
                {
                    ...
                    if(Data.hp <= 0)
                    {
                        go = false;
                        repaint();
                        setTitle("YOU WAS KILLED! YOU KILLED " + Data.kills + " BOTS BEFORE END!");
                    }
                    if(counter % 20 == 0)
                    {
                        repaint();
                    ...
                    if(counter % bot_rate_of_fire == 0)
                    {
                        setTitle("BOTS KILLED: " + Data.kills);
                        botsShoot();
                    }
    ...
    private void botsShoot()
    {
        synchronized (players)
        {
            for(Player player : players)
            {
                double temp_dx = x - player.x;
                double temp_dy = y - player.y;

                double temp_c = Math.sqrt(temp_dx * temp_dx + temp_dy * temp_dy);
                if (temp_c < bot_distance_to_shoot)
                {
                    if (random.nextBoolean())
                    {
                        synchronized (Client.temp_bullets2)
                        {
                            Client.temp_bullets2.add(new Bullet(1, 10, player.x, player.y, x, y));
                        }
                    }
                    else
                        {
                        double xr = x + random.nextInt(4 * Data.player_radius) - 2 * Data.player_radius;
                        double yr = y + random.nextInt(4 * Data.player_radius) - 2 * Data.player_radius;

                        synchronized (Client.temp_bullets2)
                        {
                            Client.temp_bullets2.add(new Bullet(1, 10, player.x, player.y, xr, yr));
                        }
                    }
                }
            }
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        Client client = new Client(1600, 1000);
    }
